...
- Annualized information ratio ≈ **−0.25**
- Mean monthly cross-sectional IC ≈ **−0.025** (t = −0.44 — indistinguishable from zero)
- Pooled IC ≈ **−0.032**; long hit rate 56%, short hit rate 49%
- Quintile forward returns are non-monotonic: the lowest-scored quintile returned **+2.78%** per period vs **+2.55%** for the highest-scored, with the fourth quintile worst at +0.44%

**The score, as currently weighted, shows no predictive edge on this universe — and I present that openly rather than quoting the earlier three-ticker pilot (IR ≈ +0.33), which the full run contradicts.** These figures are post-audit: a September 2026 review found that the statistics component had been silently dropped from every earlier backtest and that the spectral phase labels were shifted a quarter cycle; both are fixed, and the result did not change materially (IR moved from −0.38 to −0.25). The instability between the two runs demonstrates the core statistical problem: fourteen highly-correlated mega-cap survivors are effectively one factor bet, so the true sample is closer to 37 months than 518 observations, and any IC estimate is noise-dominated. A companion per-ticker timing test tells the same story — the signal-timed strategy trails SPY on 11 of 14 names (mean alpha ≈ −22%).
